define(function(){
    ...
    var initialize = function(module){
        ...
        module
            .directive('kzSidebarMenufx', ["$timeout",function($timeout){
                return {
                    restrict: 'EA'
                    , replace: true
                    , template:
                    ...
                    , controller: [function(){
                        ...
                        this.checkActive = function(name){
                            var menufxCrumbs = null;
                            if(!angular.isUndefined(window.localStorage.menufxCrumbs)){
                                menufxCrumbs = JSON.parse(window.localStorage.menufxCrumbs);
                            }
                            //console.log(menufxCrumbs)
                            var currentLoactiona =  angular.isUndefined(menufxCrumbs[0])? {"name":""} : menufxCrumbs[0];
                            //alert(currentLoaction.group)
                            return (name === currentLoactiona.name)? true: false;
                        };
